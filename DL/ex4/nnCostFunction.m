...
T1 = Theta1(:, 2:end);
T2 = Theta2(:, 2:end);
J = (-1/m) * maxtrixSum(y_matrix .* log(a3) + ...
    (1 - y_matrix) .* log(1 - a3)) + ...
    (lambda/(2 * m)) * (maxtrixSum(T1 .^ 2) + maxtrixSum(T2 .^ 2));

for i=1:m
    deltaL = a3(:, i) - y_matrix(:, i);
    delta2 = (Theta2' * deltaL) .* sigmoidGradient([1; z2(:, i)]);
    Theta1_grad = Theta1_grad + delta2(2:end) * X(i, :);
    Theta2_grad = Theta2_grad + deltaL * a2(:, i)';
end
